...
const nameAndAuthor = async function(req,res){
    const data = await BookModel.find({}).select('bookName').select('authorName')
    res.send(data)
}
const yearInput = async function(req,res){
    const yearr = req.body.year
    const data = await BookModel.find({year:{$eq : yearr}})
    res.send(data)
}

const ParticularBooks = async function(req,res){
    const data = req.body
    const savedData = await BookModel.find(data)
    res.send(savedData)

} 

const getXINRBooks = async function(req,res){
    const data = await BookModel.find({indianPrice: {$in:["100","200","500"] }})
    res.send(data)
}

const getRandomBooks = async function(req,res){
    const data = await BookModel.find({$or:[{isStockAvaialable:true },{pages:{$gt: 500}}]})
    res.send(data)
}
...
module.exports.nameAndAuthor= nameAndAuthor
module.exports.yearInput=yearInput
